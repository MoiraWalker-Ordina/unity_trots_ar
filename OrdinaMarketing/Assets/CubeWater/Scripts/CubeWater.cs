...
public class CubeWater : Water {
	...
	void CreateCube(Vector3 size, int subdivisionX, int subdivisionY, int subdivisionZ) 
	{
		...
		//
		//var downMesh = CreateGridMesh(1, 1, new Vector3(size.x, -size.y, size.z), 0);
		...
		//var wallMesh5 = CreateGridMesh(subdivisionX, subdivisionZ, size, 0, true);		var wallMesh1 = CreateGridMesh(subdivisionX, subdivisionY, size, 1);
		...
		//var wallMesh5 = CreateGridMesh(1, 1, size, 0, true);
		...
		//downWater.layer = LayerMask.NameToLayer("Water");
