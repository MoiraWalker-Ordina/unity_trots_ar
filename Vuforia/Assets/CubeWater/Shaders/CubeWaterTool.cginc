#ifndef CUBEWATERTOOL
#define CUBEWATERTOOL
...
float3 CalculateWavesDisplacement(float3 vert, float4 _QA, float4 _A, float4 _S, float4 _Dx, float4 _Dz, float4 _L)
{
	...
	float4 phase = _Dx*vert.x + _Dz*vert.z + _S*_T;
...
float ToonFresnel(float3 I, float3 N, float R0)
{
	//float icosIN = saturate(1 - dot(I, N));
	float icosIN = step(dot(I, N), 0.25);
	//float i2 = icosIN*icosIN, i4 = i2*i2;
	return R0 + (1 - R0)*(icosIN);
}
